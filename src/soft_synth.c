 ...
 *      Author: petera
 */
#define SWITCH_CASE_DOESNT_WORK
...
#ifdef SWITCH_CASE_DOESNT_WORK
	if (wf_prop == TONE)
		return &pChannel->tone;
	else if (wf_prop == FREQUENCY)
		return &pChannel->f;
	else if (wf_prop == VOLUME)
		return &pChannel->vol;
	else if (wf_prop == MODULATION_FACTOR)
		return &pChannel->mod_fact;
	else if (wf_prop == MODULATION_DELTA)
		return &pChannel->mod_f_delta;
	else if (wf_prop == NOISE_FACTOR)
		return &pChannel->noise_fact;
	else if (wf_prop == NOISE_FREQUENCY)
		return &pChannel->noise_freq;
	else if (wf_prop == HIPASS_FACTOR)
		return &pChannel->hipass_fact;
	else if (wf_prop == LOPASS_FACTOR)
		return &pChannel->lopass_fact;
	else if (wf_prop == WAVE_SEL)
		return &pChannel->lopass_fact;
	else
		return 0;
#else
...
static int synth_synthetisize(struct SYNTH_Channel_t* pChannel) {
	...
	// output mux
#ifdef SWITCH_CASE_DOESNT_WORK
	if (pChannel->wavesel == 0) {
		out = o_sq;
	} else if (pChannel->wavesel == 1) {
		out = o_sawup;
	} else if (pChannel->wavesel == 2) {
		out = o_sawdown;
	} else if (pChannel->wavesel == 3) {
		out = o_tri & (MAX_OUT-1);
	} else {
		out = 0;
	}
	...
	if (pChannel->hipass_fact) {
		out -= LINEAR_INTERPOLATION(pChannel->hipass_fact, out, zout, ACC_O);
	}

	// volume
	out = LINEAR_INTERPOLATION(pChannel->vol, 0, out, ACC_O);
